...
int fteRemote(String command);
int fteAllRemote(String command);
...
void setup() {
    ...
    Particle.function("fte", fteRemote);
    Particle.function("fteAll", fteAllRemote);
...
int fteRemote(String command){
  char inputStr[10];
  command.toCharArray(inputStr,10);
  float i = atoi(inputStr);
  int multipled_chance = i * 10;
  ...
  uint32_t startMillis = millis();
  while(millis() - startMillis < seconds*1000UL){

    for(int u = 1; u <= 11; u++){
          int randNum = rand()%(max-min + 1) + min;
      // If Hillary is higher than the random number, she wins.
      if (multipled_chance > randNum) {
          b.ledOff(u);
          delay(wait);
          // Blue
          b.smoothLedOn(u+0.5,0,0,3);
          delay(wait);
      // If she is lower than the random number, lose.
      } else {
        b.ledOff(u);
        delay(wait);
        b.smoothLedOn(u+0.5,3,0,0);
        delay(wait);
      }
    }
  }
